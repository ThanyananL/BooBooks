	...
	<div class="container-fluid">
		...
		<form action="search.php" method="get" style="width: 100%;">
			...
			<div class="input-group my-5">
				<input type="text" class="form-control" name="key" placeholder="ค้นหาหนังสือ...." aria-label="Search Book..." aria-describedby="basic-addon2">
		...
		<div class="d-flex pt-3">
			<?php if ($books == 0) { ?>
				<div class="alert alert-warning 
					...
					<br>
					ไม่มีหนังสือเล่มนี้อยู่ในระบบฐานข้อมูลของเรา
				</div>
			<?php } else { ?>
					...
					<?php foreach ($books as $book) { ?>
						<div class="card m-1">
							...
							<div class="card-body">
								...
								<a href="uploads/files/<?= $book['file'] ?>" class="btn btn-success">อ่าน</a>
								<a href="uploads/files/<?= $book['file'] ?>" class="btn btn-primary" download="<?= $book['title'] ?>">ดาวน์โหลด</a>
								<?php if (isset($_SESSION['loginuser_id'])) : ?>
									<a href="add_to_favorites.php?book_id=<?= $book['id'] ?>" class="btn btn-warning mt-2">Add to Favorites</a>
								<?php else : ?>
									<a href="loginUser.php" class="btn btn-warning mt-2">Log in to add to Favorites</a>
								<?php endif; ?>
							</div>
						</div>
					<?php } ?>
			...
			<div class="category">
				...
				<div class="list-group">
					<?php if ($categories == 0) {
						...
						<a href="#" class="list-group-item list-group-item-action active">หมวดหมู่หนังสือ</a>
						<?php foreach ($categories as $category) { ?>
				...
				<div class="list-group mt-5">
					<?php if ($authors == 0) {
						...
						<a href="#" class="list-group-item list-group-item-action active">ผู้แต่ง / สำนักงาน</a>
						<?php foreach ($authors as $author) { ?>
